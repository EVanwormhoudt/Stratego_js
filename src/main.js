...
io.on('connection', (socket) => {
    ...
    //judith socket register username
...
/*
    socket.on("passDecrypt", (info) => {
        // Load hash from your password DB.
        let sql = "SELECT password FROM users WHERE username = ?";
        con.query(sql, info[0], (err, res) => {
            if (err) throw err;
            if (res.length){}
            bcrypt.compare(myPlaintextPassword, res, function(err, result) {
                // result == true
            });
        })
    })*/
